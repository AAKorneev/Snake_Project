...
void set_snake(char** field, const size_t* snake_x, const size_t* snake_y, size_t snake_size) {
...
void clear_snake(char** field, const size_t* snake_x, const size_t* snake_y, size_t snake_size) {
	for (size_t i = 0; i < snake_size; i++)
		field[snake_y[i]][snake_x[i]] = field_symbol;
}

/* Установка еды в игровое поле field */
void set_food(char** field, size_t rows, size_t columns, size_t* food_x, size_t* food_y) {
	if (!food_flag) {
		generate_food(rows, columns, food_x, food_y);
...
void check_snake(char** field, size_t rows, size_t columns, size_t* snake_x, size_t* snake_y, DIRECTION direction) {
	if (snake_x[0] == 0)
		snake_x[0] = columns - 2;
...
void check_game(char** field, size_t* snake_x, size_t* snake_y, bool* game_on, size_t snake_size) {
...
void animate_snake(char** field, size_t rows, size_t columns, size_t* snake_x, size_t* snake_y, 
						size_t* snake_size, size_t food_x, size_t food_y, DIRECTION direction) {
	...
	check_snake(field, rows, columns, snake_x, snake_y, direction);
