...
int main() {
	...
	field = create_field_array(rows, columns);

	init_field(field, rows, columns);									// Заполнение массива игрового поля
	init_snake(rows, columns, snake_x, snake_y, snake_size, direction);	// Инициализация змейки
	init_food();														// Загушка		
	...
	while (game_on) {
		...
		set_food(field, rows, columns, &food_x, &food_y);												// Установка еды в поле
		animate_snake(field, rows, columns, snake_x, snake_y, &snake_size, food_x, food_y, direction);	// Анимация змейки
	...
	destroy_field_array(field, rows, columns);
	return 0;
